...
-- EDITED
print("The deprecated 'buildinghelper_old.lua' file was loaded!")
-- DONE
...
function BuildingHelper:AddBuilding(building)
	...
	function building:UpdateHealth(fBuildTime, bScale, fMaxScale)
		...
		--print("\tbuilding.nHealthInterval ("..building.nHealthInterval..") = "..building.nHealthInterval.." * 1 / ("..fBuildTime.."/"..BUILDINGHELPER_THINK..")") --
		...
		if bScale then
			...
			--print("Check this out: "..building.fCurrentScale)						--
    ...
    callback = function()
		if IsValidEntity(building) then
			if building.bUpdatingHealth then
				...
				if building:GetHealth() > 0 and building:GetHealth() < building.fMaxHealth and GameRules:GetGameTime() <= building.fTimeBuildingCompleted then		--
					--print("Updating health for "..building:GetUnitName()..". Current hp: "..building:GetHealth()+building.nHealthInterval)						--
					building:SetHealth(building:GetHealth()+building.nHealthInterval)
				else
					if building:GetHealth() < 0 then																		--
						--print("Building was destroyed during construction for "..building:GetUnitName())					--
						building._interrupted = true																		--
					else																									--
						--print("Finishing updating health for "..building:GetUnitName())									--
					end																										--
			...
			if building.bScale then
				if building.fCurrentScale < building.fMaxScale and building:GetHealth() > 0 then							--
					building.fCurrentScale = building.fCurrentScale+building.fScaleInterval
					--print("Scale of "..building:GetUnitName()..": "..building.fCurrentScale.." with interval "..building.fScaleInterval)	--
