...
function ModifyLumber( player, lumber_value )
   ...
   -- Edit
   local hero = GetPlayerHero(player:GetPlayerID())

   if lumber_value == 0 then return end
   if lumber_value > 0 then

      -- EDITED
      hero:IncLumber(lumber_value)
      -- END
      ...
      -- EDITED
      if hero:HasEnoughLumber(lumber_value) then
	 hero:DecLumber(math.abs(lumber_value))
...
function PlayerHasEnoughGold( player, gold_cost )
   ...
   if not hero:HasEnoughGold(gold_cost) then
...
function PlayerHasEnoughLumber( player, lumber_cost )
   ...
   -- Edit
   if hero:HasEnoughLumber(lumber_cost) == false then
